...
namespace Chimera
{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			CM_CORE_ASSERT(m_ColorAttachments.size() <= 4, "");
		...
		CM_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			CM_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
